...
final _ahora = DateTime(2026, 9, 21, 12, 0); // lunes 21 de septiembre de 2026, mediodia

Funcion _f(int id, int idPelicula, String fecha, String hora) =>
    Funcion(idFuncion: id, idPelicula: idPelicula, idSala: 1, fecha: fecha, horaInicio: hora, horaFin: '23:00:00', estado: 'programada');

Pelicula _p(int id, String titulo) => Pelicula(idPelicula: id, titulo: titulo, duracionMin: 120, estado: 'activa');

DatosLocales _datos({List<CompraHistorial> compras = const []}) => DatosLocales(
      peliculas: [_p(1, 'Oppenheimer'), _p(2, 'Barbie'), _p(3, 'Spider-Man: Beyond'), _p(4, 'El Viaje de Chihiro'), _p(5, 'Blade Runner 2049')],
      funciones: [
...
InterpreteLocal _interprete({List<CompraHistorial> compras = const []}) => InterpreteLocal(_datos(compras: compras), ahora: () => _ahora);
...
void main() {
  ...
  group('esAlucinacion (lo que Whisper inventa con ruido o eco)', () {
    test('detecta frases inventadas y bucles', () {
      expect(esAlucinacion('Subtítulos realizados por la comunidad de Amara.org'), isTrue);
      expect(esAlucinacion('La tinta de la tinta de la tinta de la tinta de la tinta.'), isTrue);
      expect(esAlucinacion('sí sí sí sí sí'), isTrue);
      expect(esAlucinacion('  '), isTrue);
    });

    test('no marca frases reales', () {
      for (final t in ['Adiós, adiós, adiós.', '¿Qué hay en cartelera?', 'Dos entradas, dos gaseosas y dos nachos']) {
  ...
  group('InterpreteLocal (sin internet)', () {
    ...
    test('horarios de una pelicula: solo las funciones que faltan, con la hora hablada', () {
      final r = _interprete().interpretar('¿A qué hora es Barbie?');
      expect(r.texto, '«Barbie» tiene funciones hoy a las ocho y media de la noche y mañana a las seis de la tarde.');
    ...
    test('entiende titulos mal escritos por el reconocedor y titulos largos', () {
      expect(_interprete().interpretar('cuando pasan operheimer').acciones.single['ids'], [1]);
      expect(_interprete().interpretar('que horarios tiene el viaje de chihiro').acciones.single['ids'], [4]);
      expect(_interprete().interpretar('funciones de blade runner').acciones.single['ids'], [5]);
    });

    test('comprar, pagar o pedir dulceria necesita conexion', () {
      for (final frase in ['quiero dos entradas para Barbie', 'quiero comprar', 'agregame unos pochoclos', 'quiero el asiento A5', 'confirmo']) {
    ...
    test('mis compras no se confunde con comprar', () {
      ...
      final compra = CompraHistorial(
        venta: const Venta(idVenta: 9, idFuncion: 10, total: 70, estado: 'pagada'),
        pelicula: 'Barbie',
        sala: 'Sala 1',
        fecha: '2026-09-21',
        horaInicio: '20:30:00',
    ...
    test('funciones del dia y de mañana', () {
      expect(_interprete().interpretar('¿qué funciones hay hoy?').texto, startsWith('hoy hay 2 funciones'));
      expect(_interprete().interpretar('funciones de mañana').texto, contains('Barbie a las seis de la tarde'));
    });
    ...
    test('saludo, gracias, adios, inicio y algo que no entiende', () {
      ...
      expect(_interprete().interpretar('menú principal').acciones.single['destino'], 'inicio');
      expect(_interprete().interpretar('blablabla').texto, contains('Sin conexión solo puedo'));
    });

    test('sin copia de la cartelera lo dice en vez de inventar', () {
      final r = InterpreteLocal(const DatosLocales(), ahora: () => _ahora).interpretar('qué hay en cartelera');
  ...
  group('CacheLocal', () {
    ...
    test('guarda y lee la copia con su fecha', () async {
      await CacheLocal.instance.guardar('peliculas', [_p(1, 'Barbie').toJson()]);
      final copia = await CacheLocal.instance.leer('peliculas');
      expect(copia, isNotNull);
      expect(Pelicula.fromJson((copia!.datos as List).first as Map<String, dynamic>).titulo, 'Barbie');
      expect(DateTime.now().difference(copia.guardado).inSeconds, lessThan(5));
    });

    test('lo que no existe o esta roto no lanza: simplemente no hay copia', () async {
      expect(await CacheLocal.instance.leer('nada'), isNull);
      await File('${carpeta.path}/roto.json').writeAsString('{ esto no es json');
    ...
    test('los modelos ida y vuelta por JSON (asi se guardan las copias)', () {
      ...
      final c = CompraHistorial(venta: v, pelicula: 'Barbie', sala: 'Sala 2', fecha: '2026-09-21', horaInicio: '20:30:00');
      final otra = CompraHistorial.fromJson(jsonDecode(jsonEncode(c.toJson())) as Map<String, dynamic>);
  ...
  group('catalogo de modelos', () {
    test('los tamaños son los de los archivos reales y el total ronda lo anunciado', () {
      ...
      final recomendado = paquete('stt_tiny').bytesTotal + paquete('tts').bytesTotal;
      expect(recomendado / 1024 / 1024, inInclusiveRange(180, 215));
    });

    test('los archivos grandes traen su sha256 para verificarlos', () {
      for (final a in paquete('stt_tiny').archivos.where((a) => a.bytes > 5 * 1024 * 1024)) {
  ...
  group('GestorModelos: descarga, verificacion, reanudacion y borrado', () {
    ...
    var cortarPrimeraDescargaEn = -1; // bytes tras los que el servidor corta la conexion (una sola vez)
    var pedidos = <String>[];
    var rangosRecibidos = <String>[];

    List<int> bytesDe(int n, int semilla) => List<int>.generate(n, (i) => (i * 31 + semilla) & 0xff);

    PaqueteModelo paqueteDePrueba(String id, Map<String, List<int>> contenido, {bool shaMalo = false}) => PaqueteModelo(
          id: id,
          nombre: id,
          descripcion: '',
          archivos: [
            for (final e in contenido.entries)
              ArchivoDescarga(
                ...
                sha256: e.value.length > 1000 ? (shaMalo ? '0' * 64 : sha256.convert(e.value).toString()) : null,
              ),
          ],
        );

    GestorModelos gestor(List<PaqueteModelo> paquetes, {List<ConnectivityResult> red = const [ConnectivityResult.wifi]}) =>
        GestorModelos(raiz: raiz, paquetes: paquetes, conectividad: () async => red);
    ...
    setUp(() async {
      ...
      servidor.listen((req) async {
        ...
        if (rango != null) {
          ...
          req.response.headers.set('content-range', 'bytes $desde-${datos.length - 1}/${datos.length}');
        }
        final cuerpo = datos.sublist(desde);
        if (cortarPrimeraDescargaEn >= 0 && cuerpo.length > cortarPrimeraDescargaEn && desde == 0) {
          ...
          await socket.flush();
          await Future<void>.delayed(const Duration(milliseconds: 50)); // deja que llegue antes de cortar
          socket.destroy();
          return;
        }
    ...
    PaqueteModelo stt() => paqueteDePrueba('stt_tiny', {'stt/encoder.onnx': archivos['stt/encoder.onnx']!, 'stt/tokens.txt': archivos['stt/tokens.txt']!});
    ...
    test('descarga todo, lo verifica y el modo sin conexion queda listo', () async {
      ...
      expect(await File('${raiz.path}/stt_tiny/stt/encoder.onnx').readAsBytes(), archivos['stt/encoder.onnx']);
      expect(await File('${raiz.path}/tts/tts/espeak/b_dict').readAsBytes(), archivos['tts/espeak/b_dict']);
      expect(Directory(raiz.path).listSync(recursive: true).where((e) => e.path.endsWith('.part')), isEmpty);
    });
    ...
    test('si se corta la conexion queda pausado y "reanudar" sigue donde quedo (pedido Range)', () async {
      ...
      expect(yaBajado, inInclusiveRange(1, 120000)); // lo que alcanzo a llegar antes del corte

      await g.descargar('stt_tiny'); // reanudar
      expect(g.estado('stt_tiny'), EstadoPaquete.listo);
      expect(rangosRecibidos, contains('bytes=$yaBajado-')); // pidio SOLO lo que faltaba
      expect(await File('${raiz.path}/stt_tiny/stt/encoder.onnx').readAsBytes(), archivos['stt/encoder.onnx']); // integro
    });

    test('un archivo dañado (huella distinta) se rechaza y se borra', () async {
      final g = gestor([paqueteDePrueba('stt_tiny', {'stt/encoder.onnx': archivos['stt/encoder.onnx']!}, shaMalo: true), tts()]);
      await g.iniciar();
      await g.descargar('stt_tiny');
      expect(g.estado('stt_tiny'), EstadoPaquete.error);
      expect(g.error('stt_tiny'), contains('dañado'));
      expect(await File('${raiz.path}/stt_tiny/stt/encoder.onnx').exists(), isFalse);
      expect(await File('${raiz.path}/stt_tiny/stt/encoder.onnx.part').exists(), isFalse);
    });

    test('un archivo que llega incompleto se rechaza', () async {
      final incompleto = paqueteDePrueba('stt_tiny', {'stt/encoder.onnx': archivos['stt/encoder.onnx']!});
      // El catalogo dice que pesa mas de lo que el servidor entrega.
      final mentiroso = PaqueteModelo(
        id: 'stt_tiny',
        nombre: 'x',
        descripcion: '',
        archivos: [ArchivoDescarga(url: incompleto.archivos.first.url, ruta: 'stt/encoder.onnx', bytes: 400000)],
      );
    ...
    test('borrar libera el espacio y deja el modelo como no descargado', () async {
      final g = gestor([stt(), tts()]);
      await g.iniciar();
      await g.descargarRecomendados();
    ...
    test('los ajustes (solo Wi‑Fi, preferir el telefono, reconocimiento elegido) se recuerdan', () async {
      final g = gestor([stt(), tts()]);
      await g.iniciar();
      g.setSoloWifi(false);
