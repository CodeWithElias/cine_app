...
class CompraView extends StatelessWidget {
  ...
  final void Function(int idVenta, String evento, [String? mensaje])?
  onPagoEvento;
  ...
  Widget build(BuildContext context) {
    return ListenableBuilder(
      listenable: cine,
      builder: (context, _) {
        if (cine.peliculaSeleccionada == null ||
            cine.funcionSeleccionada == null) {
            ...
            Expanded(
              child: switch (cine.estadoCompra) {
                ...
                _ => PagoView(
                  ...
                  ui: ui,
                  pagos: pagos,
                  onCampos: onPagoCampos,
                  onEvento: onPagoEvento,
                ),
              },
            ),
            if (cine.estadoCompra != EstadoCompra.pago)
              _BarraInferior(cine: cine),
          ],
        );
      },
    );
...
TextStyle _etiqueta(Color c) => TextStyle(
  color: c,
  fontSize: 11,
    ...
    child: Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          const Text(
            'Aún no elegiste una función.\nPídele una película al asistente o elígela en la cartelera.',
            textAlign: TextAlign.center,
            style: TextStyle(color: LumenColors.onSurfaceVariant, height: 1.4),
          ),
        ...
        children: [
          Row(
            children: [
              Expanded(
                child: Text(
                  cine.butacas.isEmpty
                      ? 'Sin asientos elegidos'
                      : 'Asientos: ${cine.butacas.map((b) => b.id).join(', ')}',
                  style: const TextStyle(
              ...
              Expanded(
                child: FilledButton(
                  onPressed: puedeSeguir
                      ? () => cine.setEstadoCompra(
                          enAsientos
                              ? EstadoCompra.seleccionandoCandybar
                              : EstadoCompra.pago,
                        )
                      : null,
                  child: Text(seguir),
  ...
  Widget build(BuildContext context) {
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 8),
            Text(
              cine.errorDisponibilidad != null
                  ? 'No se pudieron cargar las butacas.\n${cine.errorDisponibilidad}'
                  : 'No hay butacas para mostrar todavía.',
              textAlign: TextAlign.center,
              style: const TextStyle(color: LumenColors.onSurfaceVariant),
            ),
            TextButton(
              onPressed: cine.recargarDisponibilidad,
    ...
    return LayoutBuilder(
      builder: (context, c) {
        ...
        final lado = ((disponible - espacio * (maxPorFila - 1)) / maxPorFila)
            .clamp(24.0, 38.0);
        return SingleChildScrollView(
          ...
          child: Column(
            children: [
              ...
              // Si la sala tiene tantas butacas por fila que no entran, el mapa entero se achica en vez de desbordarse.
              FittedBox(
                        ...
                        child: Row(
                          ...
                          children: [
                            ...
                            for (final a in filas[fila]!)
                              Padding(
                                padding: const EdgeInsets.only(right: espacio),
                                child: _Butaca(a: a, lado: lado, cine: cine),
                              ),
                          ],
                        ),
                      ),
              ...
              const Wrap(
                ...
                children: [
                  ...
                  _Leyenda(
                    color: LumenColors.primaryContainer,
                    texto: 'Elegido',
                  ),
                  _Leyenda(
    ...
    if (!a.disponible) {
      return Container(
        ...
        child: Icon(
          Icons.close,
          size: lado * 0.5,
          color: LumenColors.outlineVariant,
        ),
      );
    }
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          ...
          boxShadow: elegida
              ? [
                  BoxShadow(
                    color: LumenColors.primaryContainer.withValues(alpha: 0.55),
                    blurRadius: 12,
                  ),
                ]
              : const [],
        ...
        child: elegida
            ? Icon(
                Icons.check,
                size: lado * 0.55,
                color: LumenColors.onPrimaryContainer,
              )
            : Text(
                '${a.numero}',
                style: TextStyle(
...
class _DulceriaState extends State<_Dulceria> {
  ...
  Future<void> _cargar() async {
    ...
    try {
      final r = await Future.wait<Object>([
        widget.cine.api.listarCategoriasDulceria(),
        widget.cine.api.listarProductosDulceria(),
      ]);
      if (!mounted) return;
      setState(() {
        _categorias = r[0] as List<CategoriaDulceria>;
        _productos = (r[1] as List<ProductoDulceria>)
      ...
      setState(() {
        _error =
            'No se pudo cargar el menú de dulcería. Puedes saltar este paso y seguir con el pago.';
        _cargando = false;
      });
    }
  }

  int _cantidad(ProductoDulceria p) =>
      widget.cine.dulceria
          .where((i) => i.id == '${p.idProducto}')
          .firstOrNull
          ?.cantidad ??
      0;
  ...
  void _cambiar(ProductoDulceria p, int delta) {
    ...
    widget.cine.actualizarDulceria(
      ItemDulceria(
        id: '${p.idProducto}',
        nombre: p.nombre,
  ...
  Widget build(BuildContext context) {
    ...
    final visibles = _categoria == null
        ? _productos
        : _productos.where((p) => p.idCategoria == _categoria).toList();
          ...
          child: ListView(
            ...
            children: [
              ...
              for (final c in _categorias)
                _chip(
                  c.nombre,
                  _categoria == c.idCategoria,
                  () => setState(() => _categoria = c.idCategoria),
                ),
            ],
          ),
        ),
  ...
  Widget _fila(ProductoDulceria p) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(
          color: cantidad > 0
              ? LumenColors.primary.withValues(alpha: 0.6)
              : Colors.transparent,
        ),
      ),
      child: Row(
      ...
      child: Column(
        children: [
                  ...
                  decoration: BoxDecoration(
                    ...
                    gradient: const LinearGradient(
                      colors: [
                        ...
                        LumenColors.tertiary,
                      ],
                    ),
                  ),
                  child: const Icon(
                    Icons.verified,
                    color: LumenColors.onTertiary,
                    size: 30,
                  ),
                ),
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        venta?.metodoPagoElegido == 'stripe'
          ...
          EntradaDigital(
            ...
            dulceria: [
              for (final i in cine.dulceria) '${i.cantidad} × ${i.nombre}',
            ],
...
class EntradaDigital extends StatelessWidget {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        gradient: LinearGradient(
          ...
          colors: [
            LumenColors.primaryContainer.withValues(alpha: 0.25),
            LumenColors.surfaceContainerHigh,
            LumenColors.secondaryContainer.withValues(alpha: 0.2),
          ],
        ),
      ),
      child: Column(
        ...
        children: [
          Text(
            'LUMEN CINEMA · ${(sala ?? 'SALA POR CONFIRMAR').toUpperCase()}',
            style: _etiqueta(LumenColors.secondary),
          ),
          const SizedBox(height: 4),
          Text(
            pelicula,
  ...
  Widget _dato(
    String etiqueta,
    String valor, {
    Color color = LumenColors.onSurface,
    bool derecha = false,
  }) => Column(
    crossAxisAlignment: derecha
