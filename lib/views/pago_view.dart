...
Map<String, Map<String, dynamic>> camposDeTarjeta(CardFieldInputDetails? d) {
  Map<String, dynamic> campo(CardValidationState? estado, bool escrito, String mensaje) => {
        'completo': estado == CardValidationState.Valid,
        'vacio': !escrito && estado != CardValidationState.Valid && estado != CardValidationState.Invalid,
        'error': estado == CardValidationState.Invalid ? mensaje : null,
      };
  return {
    'numero': campo(d?.validNumber, (d?.last4 ?? '').isNotEmpty || d?.validNumber == CardValidationState.Invalid, 'El número de tarjeta no es válido.'),
    'vencimiento': campo(d?.validExpiryDate, d?.expiryMonth != null || d?.validExpiryDate == CardValidationState.Invalid, 'La fecha de vencimiento no es válida.'),
    'cvc': campo(d?.validCVC, d?.validCVC == CardValidationState.Invalid, 'El código de seguridad no es válido.'),
  };
}
...
class PagoView extends StatefulWidget {
  ...
  const PagoView({super.key, required this.cine, required this.ui, required this.pagos, this.onCampos, this.onEvento});
...
class _PagoViewState extends State<PagoView> {
  ...
  late _Metodo _metodo = widget.pagos.habilitado ? _Metodo.tarjeta : _Metodo.efectivo;
  bool _acepta = false;
  ...
  Timer? _pausaCampos;
  late int _atendido;
  late final TextEditingController _titular = TextEditingController(text: (AuthService.instance.usuario?.nombre ?? '').toUpperCase());
  ...
  bool get _camposCompletos => _campos.values.every((c) => c['completo'] == true);

  @override
  void initState() {
    super.initState();
    _atendido = widget.ui.solicitudPago?.n ?? 0; // los pedidos anteriores a que se abriera esta pantalla no valen
  ...
  void _alCambiarTarjeta(CardFieldInputDetails? detalle) {
    ...
    _pausaCampos?.cancel();
    _pausaCampos = Timer(const Duration(milliseconds: 250), () => widget.onCampos?.call(id, _campos));
  }

  // ------------------------------------------------------------------ crear la venta (una sola vez)

  Future<int> _obtenerIdVenta() async {
    if (_cine.pendienteVigente) return _cine.ventaPendiente!.idVenta; // reintento tras un rechazo: no se crea otra venta
    if (_cine.ventaPendiente != null) {
      ...
      await _api.cancelarVentaPendiente(_cine.ventaPendiente!.idVenta).catchError((Object _) {});
      _cine.limpiarPendiente();
    }
    final f = _cine.funcionSeleccionada;
    if (f == null) throw _PagoFallido('error', 'No hay una función elegida.');
    final venta = await _api.crearVenta(idFuncion: f.idFuncion, idAsientos: _cine.butacas.map((b) => b.idAsiento).toList(), dulceria: _cine.dulceria);
  ...
  Future<Venta> _cobrarConTarjeta(int idVenta) async {
    ...
    try {
      ...
      if (e.status == 409) {
        _cine.limpiarPendiente();
        throw _PagoFallido('error', 'Tu reserva venció o ya no está disponible. Vuelve a elegir tus asientos.');
      }
      rethrow;
    }
    ...
    try {
      await Stripe.instance.confirmPayment(
        ...
        data: PaymentMethodParams.card(
          paymentMethodData: PaymentMethodData(billingDetails: BillingDetails(name: _titular.text.trim().isEmpty ? null : _titular.text.trim())),
        ),
      );
    } on StripeException catch (e) {
      // Que el servidor tambien registre el rechazo (el webhook puede tardar); si esta consulta falla no importa.
      unawaited(_api.verificarPagoStripe(inicio.idPago).then((_) {}, onError: (Object _) {}));
      throw _PagoFallido('rechazado', e.error.localizedMessage ?? e.error.message ?? 'La tarjeta fue rechazada.');
    }
    ...
    for (var intento = 0; intento < 6; intento++) {
      ...
      if (v.estado == 'fallido' || v.estado == 'cancelado') {
        throw _PagoFallido('rechazado', v.mensaje ?? 'El pago no se pudo completar.');
      }
      await Future<void>.delayed(const Duration(milliseconds: 1200));
    }
    throw _PagoFallido('error', 'Stripe todavía no confirmó el pago. Espera unos segundos y vuelve a tocar Pagar.');
  }
  ...
  Future<void> _pagar() async {
    ...
    if (idVoz == null && !_acepta) {
      setState(() {
        _aviso = 'Para continuar tienes que aceptar que las entradas no tienen reembolso.';
        _avisoEsError = true;
      });
      return;
    }
    if (_metodo == _Metodo.tarjeta && !_camposCompletos) {
      setState(() {
        _aviso = 'Completa el número, el vencimiento y el código de seguridad de la tarjeta.';
        _avisoEsError = true;
      });
      return;
    }
    ...
    try {
      ...
      final venta = _metodo == _Metodo.efectivo ? await _api.pagarEnEfectivo(idVenta) : await _cobrarConTarjeta(idVenta);
      ...
      final mensaje = e is _PagoFallido ? e.mensaje : (e is ApiException ? e.message : 'No se pudo completar el pago. Intenta de nuevo.');
      if (mounted) {
        setState(() {
          _aviso = mensaje;
          _avisoEsError = true;
      ...
      _avisar(idVoz ?? _cine.ventaPendiente?.idVenta, e is _PagoFallido ? e.evento : 'error', mensaje);
    } finally {
      _enCurso = false;
    }
  }
  ...
  Future<void> _cancelar({required bool porVoz}) async {
    ...
    if (pendiente != null) {
      ...
      try {
        ...
        final texto = e is ApiException ? e.message : 'No se pudo cancelar el pago.';
        if (mounted) {
          setState(() {
            _aviso = texto;
            _avisoEsError = true;
        ...
        if (pendiente.tipoRegistro == 'voz') _avisar(pendiente.idVenta, 'error', texto);
        _enCurso = false;
        return;
      }
      _enCurso = false;
      _cine.limpiarPendiente();
      if (pendiente.tipoRegistro == 'voz') _avisar(pendiente.idVenta, 'cancelado');
      if (mounted) {
        setState(() {
          _aviso = 'Cancelaste el pago. Tus asientos volvieron a quedar libres.';
          _avisoEsError = false;
        });
      }
    }
  ...
  Widget build(BuildContext context) {
    return ListenableBuilder(
      ...
      builder: (context, _) {
        ...
        final total = vigente?.total ?? _cine.total; // con la venta creada manda su total real (incluye promociones)
        final ocupado = _fase != _Fase.editando;
        final aceptado = _porVoz || _acepta; // por voz ya se acepto al decir "confirmo" (RF03/RF19)
        final campoGuia = (_porVoz && _fase == _Fase.editando && _metodo == _Metodo.tarjeta)
            ? const {'numero': 'el número', 'vencimiento': 'la fecha de vencimiento', 'cvc': 'el código de seguridad'}
                .entries
                .where((e) => _campos[e.key]!['completo'] != true)
                .firstOrNull
                ?.value
            : null;
        final bloqueado = ocupado || !aceptado || (_metodo == _Metodo.tarjeta && !_camposCompletos);
        ...
        return SingleChildScrollView(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            if (!_porVoz)
              CheckboxListTile(
                value: _acepta,
                onChanged: ocupado ? null : (v) => setState(() => _acepta = v ?? false),
                controlAffinity: ListTileControlAffinity.leading,
                contentPadding: EdgeInsets.zero,
                activeColor: LumenColors.primary,
                title: const Text('Entiendo que las entradas no tienen reembolso.', style: TextStyle(color: LumenColors.onSurface, fontSize: 13)),
              ),
            if (_aviso != null) _cajaAviso(),
            const SizedBox(height: 6),
            Row(children: [
              OutlinedButton(
                onPressed: _enCurso || ocupado ? null : () => _cancelar(porVoz: false),
              ...
              Expanded(
                child: FilledButton(
                  ...
                  child: _fase == _Fase.procesando
                      ...
                      : Text(_metodo == _Metodo.tarjeta ? 'Pagar ${bs(total)}' : 'Confirmar pago en efectivo'),
                ),
              ),
            ]),
            const SizedBox(height: 6),
            const Text('También puedes decirle al asistente «pagar» o «cancelar».',
                textAlign: TextAlign.center, style: TextStyle(color: LumenColors.onSurfaceVariant, fontSize: 11)),
          ]),
        );
      },
    );
  ...
  Widget _encabezado() => Row(children: [
        Container(
          width: 38,
          height: 38,
          decoration: BoxDecoration(color: LumenColors.primaryContainer.withValues(alpha: 0.15), borderRadius: BorderRadius.circular(12)),
          child: const Icon(Icons.lock_outline, color: LumenColors.primary, size: 20),
        ),
        const SizedBox(width: 10),
        const Expanded(
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text('Pago seguro', style: TextStyle(color: LumenColors.onSurface, fontSize: 18, fontWeight: FontWeight.w800)),
            Text('LUMEN CHECKOUT', style: TextStyle(color: LumenColors.onSurfaceVariant, fontSize: 10, letterSpacing: 1.4)),
          ]),
        ),
        if (_porVoz)
          Container(
            padding: const EdgeInsets.symmetric(horizontal: 9, vertical: 4),
            decoration: BoxDecoration(color: LumenColors.primaryContainer.withValues(alpha: 0.15), borderRadius: BorderRadius.circular(14)),
            child: const Row(mainAxisSize: MainAxisSize.min, children: [
              Icon(Icons.graphic_eq, size: 14, color: LumenColors.primary),
              SizedBox(width: 4),
              Text('Lumen te guía', style: TextStyle(color: LumenColors.primary, fontSize: 11, fontWeight: FontWeight.w700)),
            ]),
          ),
      ]);
  ...
  Widget _resumen(double total) => Container(
        ...
        decoration: BoxDecoration(color: LumenColors.surfaceContainer, borderRadius: BorderRadius.circular(16)),
        child: Column(children: [
          _linea('Entradas (${_cine.butacas.length})', bs(_cine.totalEntradas)),
          for (final i in _cine.dulceria) _linea('${i.cantidad} × ${i.nombre}', bs(i.precio * i.cantidad)),
  ...
  Widget _linea(String a, String b, {bool fuerte = false}) => Padding(
        ...
        child: Row(children: [
          Expanded(child: Text(a, style: TextStyle(color: fuerte ? LumenColors.onSurface : LumenColors.onSurfaceVariant, fontWeight: fuerte ? FontWeight.w700 : FontWeight.w400))),
          Text(b, style: TextStyle(color: fuerte ? LumenColors.primary : LumenColors.onSurface, fontWeight: fuerte ? FontWeight.w800 : FontWeight.w600, fontSize: fuerte ? 17 : 14)),
        ]),
      );
        ...
        child: GestureDetector(
          ...
          child: AnimatedContainer(
            ...
            decoration: BoxDecoration(
              color: activo ? LumenColors.primary.withValues(alpha: 0.14) : LumenColors.surfaceContainer,
              borderRadius: BorderRadius.circular(14),
              border: Border.all(color: activo ? LumenColors.primary : Colors.transparent, width: 1.5),
            ),
            child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
              Icon(icono, size: 19, color: activo ? LumenColors.primary : LumenColors.onSurfaceVariant),
              const SizedBox(width: 7),
              Text(texto, style: TextStyle(color: activo ? LumenColors.primary : LumenColors.onSurface, fontWeight: FontWeight.w700)),
            ]),
          ),
        ),
      );
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          if (campoGuia != null)
            Container(
              ...
              decoration: BoxDecoration(color: LumenColors.primaryContainer.withValues(alpha: 0.14), borderRadius: BorderRadius.circular(12)),
              child: Text('Ahora: $campoGuia', style: const TextStyle(color: LumenColors.primary, fontWeight: FontWeight.w700, fontSize: 13)),
            ),
          const Text('TITULAR', style: TextStyle(color: LumenColors.onSurfaceVariant, fontSize: 10, fontWeight: FontWeight.w700, letterSpacing: 0.8)),
          ...
          const SizedBox(height: 10),
          const Text('DATOS DE LA TARJETA', style: TextStyle(color: LumenColors.onSurfaceVariant, fontSize: 10, fontWeight: FontWeight.w700, letterSpacing: 0.8)),
          const SizedBox(height: 4),
          ...
          if (widget.pagos.modoPrueba) ...[
            ...
            Container(
              ...
              decoration: BoxDecoration(color: LumenColors.secondaryContainer.withValues(alpha: 0.15), borderRadius: BorderRadius.circular(12)),
              child: const Text(
                ...
                style: TextStyle(color: LumenColors.secondary, fontSize: 12, height: 1.3),
              ),
            ),
          ],
          if (_campos.values.any((c) => c['error'] != null))
            Padding(
              ...
              child: Text(_campos.values.firstWhere((c) => c['error'] != null)['error'] as String, style: const TextStyle(color: LumenColors.error, fontSize: 12)),
            ),
        ]),
      );
  ...
  Widget _efectivo() => Container(
        ...
        child: const Row(children: [
          ...
          Expanded(
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text('Pago en efectivo', style: TextStyle(color: LumenColors.onSurface, fontWeight: FontWeight.w700)),
              Text('Tu compra queda registrada y pagas en boletería.', style: TextStyle(color: LumenColors.onSurfaceVariant, fontSize: 12, height: 1.3)),
            ]),
          ),
        ]),
      );
  ...
  Widget _exito() => Container(
        ...
        decoration: BoxDecoration(color: LumenColors.tertiaryContainer.withValues(alpha: 0.18), borderRadius: BorderRadius.circular(16)),
        child: const Column(children: [
          Icon(Icons.check_circle, size: 46, color: LumenColors.tertiary),
          SizedBox(height: 8),
          Text('¡Pago exitoso!', style: TextStyle(color: LumenColors.tertiary, fontSize: 18, fontWeight: FontWeight.w800)),
        ]),
      );
  ...
  Widget _cajaAviso() => Container(
        ...
        padding: const EdgeInsets.all(10),
        decoration: BoxDecoration(
          color: (_avisoEsError ? LumenColors.errorContainer : LumenColors.surfaceContainerHigh).withValues(alpha: _avisoEsError ? 0.35 : 1),
          borderRadius: BorderRadius.circular(12),
        ),
        child: Text(_aviso!, style: TextStyle(color: _avisoEsError ? LumenColors.error : LumenColors.onSurface, fontSize: 13)),
      );
}
