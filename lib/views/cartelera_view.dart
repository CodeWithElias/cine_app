...
class _CarteleraViewState extends State<CarteleraView> {
  ...
  int?
  _scrollPendiente; // pelicula a la que hay que desplazarse en cuanto exista su tarjeta
  ...
  void dispose() {
    ...
    widget.ui.filtro =
        null; // al salir, el filtro del agente no debe reaparecer la proxima vez
  ...
  Future<void> _cargar() async {
    ...
    try {
      ...
      final peliculas = (resultados[0] as List<Pelicula>)
          .where((p) => p.estado == 'activa')
          .toList();
      final ahora = DateTime.now();
      final futuras =
          (resultados[1] as List<Funcion>).where((f) {
        ...
        final visibles = widget.cine.peliculas
            .where(
              (p) =>
                  ...
                  (p.genero ?? '').toLowerCase().contains(q),
            )
            .toList();
        _seguirResaltado();
      ...
      children: [
        ...
        if (cantidad > 0 && !_cargando)
          Padding(
            ...
            child: Text(
              'Mostrando $cantidad ${cantidad == 1 ? 'título' : 'títulos'}',
              style: _etiqueta(LumenColors.onSurfaceVariant),
            ),
          ),
      ],
    );
      ...
      child: Row(
        children: [
          ...
          for (final d in _dias)
            chip(diaCorto(d), _dia == d, () => setState(() => _dia = d)),
        ],
      ),
    );
  }
  ...
  Widget _tarjeta(Pelicula p) {
    ...
    final funciones = (_funciones[p.idPelicula] ?? const <Funcion>[])
        .where((f) => _dia == null || f.fecha == _dia)
        .toList();
    final clave = _claves.putIfAbsent(p.idPelicula, () => GlobalKey());
    final anchoPortada = (Responsivo.de(context).anchoContenido * 0.26).clamp(
      76.0,
      130.0,
    );
    ...
    return AnimatedContainer(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(
          color: senalada ? LumenColors.primary : Colors.transparent,
          width: 2,
        ),
        boxShadow: senalada
            ? [
                BoxShadow(
                  color: LumenColors.primaryContainer.withValues(alpha: 0.45),
                  blurRadius: 28,
                ),
              ]
            : const [],
              ...
              children: [
                ...
                if (funciones.isEmpty)
                  ...
                  Wrap(
                    spacing: 6,
                    runSpacing: 6,
                    children: [for (final f in funciones) _horario(p, f, r)],
                  ),
              ],
            ),
          ),
  ...
  Widget _pildora(String t) => Container(
    ...
    decoration: BoxDecoration(
      color: LumenColors.surfaceContainerHighest,
      borderRadius: BorderRadius.circular(6),
    ),
    child: Text(
      t,
      style: const TextStyle(color: LumenColors.onSurfaceVariant, fontSize: 11),
    ),
  );
