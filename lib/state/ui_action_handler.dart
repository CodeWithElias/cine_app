...
class UiActionHandler {
  ...
  void aplicar(List<dynamic> acciones, int? version) {
    _cola = _cola.then((_) => _ejecutar(acciones)).catchError((Object e) {
    ...
    for (final cruda in acciones) {
      ...
      switch (a['tipo']) {
        ...
        case 'compra.seleccionar_funcion':
          ui.cerrar(TipoVentana.ticket); // arranca otra compra: el comprobante anterior ya no corresponde
          final pelicula = Pelicula.fromJson((a['pelicula'] as Map).cast<String, dynamic>());
          final funcion = Funcion.fromJson((a['funcion'] as Map).cast<String, dynamic>());
          cine.seleccionarFuncion(pelicula, funcion);
        case 'compra.seleccionar_butacas':
          cine.setButacas([
            for (final b in (a['butacas'] as List? ?? const [])) Butaca.fromJson((b as Map).cast<String, dynamic>()),
          ]);
        case 'compra.candybar':
          cine.setDulceria([
            for (final i in (a['items'] as List? ?? const [])) ItemDulceria.fromJson((i as Map).cast<String, dynamic>()),
          ]);
        case 'pago.abrir':
          ...
          final venta = a['venta'];
          if (venta is Map) cine.setVentaPendiente(Venta.fromJson(venta.cast<String, dynamic>()));
        ...
        case 'compra.completada':
          ui.cerrar(TipoVentana.confirmacion);
          final v = a['venta'];
          cine.completarCompra(v is Map ? Venta.fromJson(v.cast<String, dynamic>()) : null);
          cine.irA(Pantalla.compra);
        ...
        case 'cartelera.filtrar':
          ui.filtrarCartelera(busqueda: a['busqueda'] as String?, dia: a['dia'] as String?);
        case 'cartelera.mostrar':
          // Si ya esta en los asientos y solo cambia la funcion, no se lo saca de ahi.
          if (a['omitir_en_compra'] == true && cine.pantalla == Pantalla.compra) break;
          ui.cerrar(TipoVentana.ticket);
          ui.filtrarCartelera(); // que ningun filtro anterior oculte lo que se va a mostrar
          ui.resaltarCartelera(
            [for (final id in (a['ids'] as List? ?? const [])) (id as num).toInt()],
            (a['idFuncion'] as num?)?.toInt(),
          );
          cine.irA(Pantalla.cartelera);
          final pausa = (a['pausa_ms'] as num?)?.toInt() ?? 0;
          if (pausa > 0) await Future<void>.delayed(Duration(milliseconds: pausa)); // se ve la eleccion un momento
        case 'ventana.abrir':
          final tipo = _tipoVentana(a['ventana']);
          if (tipo != null) ui.abrir(tipo, ((a['datos'] as Map?) ?? const {}).cast<String, dynamic>());
        case 'ventana.cerrar':
          final tipo = _tipoVentana(a['ventana']);
