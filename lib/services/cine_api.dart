...
class CineApi {
  ...
  static final ValueNotifier<DateTime?> datosGuardadosDesde =
  ...
  Future<List<T>> _listaConCopia<T>(
    String clave,
    String ruta,
    ...
    try {
      ...
      if (e.status != null) {
        rethrow; // el servidor contesto (401, 500...): no es falta de conexion
      }
  ...
  dynamic _leer(http.Response r) {
    ...
    if (r.statusCode == 401 && AuthService.instance.token != null) {
  ...
  Future<dynamic> _get(String ruta) async {
    try {
      return _leer(
        await http
            .get(Uri.parse('$kBackendApiUrl$ruta'), headers: _headers())
            .timeout(_timeout),
      );
    } on ApiException {
      rethrow;
  ...
  List<Map<String, dynamic>> _lista(dynamic d) =>
      (d as List).map((e) => (e as Map).cast<String, dynamic>()).toList();

  // ---- catalogo

  Future<List<Pelicula>> listarPeliculas() =>
      _listaConCopia('peliculas', '/peliculas', Pelicula.fromJson);

  Future<List<Funcion>> listarFunciones() =>
      _listaConCopia('funciones', '/funciones', Funcion.fromJson);

  Future<List<AsientoDisponibilidad>> obtenerDisponibilidad(
    int idFuncion,
  ) async => _lista(
    await _get('/funciones/$idFuncion/disponibilidad'),
  ).map(AsientoDisponibilidad.fromJson).toList();
  ...
  Future<Sala> obtenerSala(int idSala) async => Sala.fromJson(
    ((await _get('/salas/$idSala')) as Map).cast<String, dynamic>(),
  );

  // ---- dulceria

  Future<List<CategoriaDulceria>> listarCategoriasDulceria() => _listaConCopia(
    'dulceria_categorias',
    '/dulceria/categorias',
    CategoriaDulceria.fromJson,
  );

  Future<List<ProductoDulceria>> listarProductosDulceria() => _listaConCopia(
    'dulceria_productos',
    '/dulceria/productos',
    ProductoDulceria.fromJson,
  );

  // ---- ventas y pagos

  Future<List<CompraHistorial>> listarMisCompras() => _listaConCopia(
    'compras_${AuthService.instance.usuario?.idUsuario ?? 0}',
    '/ventas',
    CompraHistorial.fromJson,
  );

  Future<DetalleCompra> obtenerDetalleCompra(int idVenta) async =>
      DetalleCompra.fromJson(
        ((await _get('/ventas/$idVenta')) as Map).cast<String, dynamic>(),
      );
  ...
  Future<Venta> crearVenta({
    ...
    final d = await _post('/ventas', {
      ...
      'dulceria': dulceria
          .map((i) => {'idProducto': int.parse(i.id), 'cantidad': i.cantidad})
          .toList(),
    });
  ...
  Future<ConfigPagos> obtenerConfiguracionPagos() async => ConfigPagos.fromJson(
    ((await _get('/pagos/config')) as Map).cast<String, dynamic>(),
  );
  ...
  Future<InicioPagoStripe> iniciarPagoStripe(int idVenta) async =>
      InicioPagoStripe.fromJson(
        ((await _post('/pagos/stripe/iniciar', {'idVenta': idVenta})) as Map)
            .cast<String, dynamic>(),
      );
  ...
  Future<VerificacionPago> verificarPagoStripe(int idPago) async =>
      VerificacionPago.fromJson(
        ((await _post('/pagos/$idPago/verificar')) as Map)
            .cast<String, dynamic>(),
      );
